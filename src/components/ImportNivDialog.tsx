...
interface ImportNivDialogProps {
  empresaCodigo: string;
}

const ImportNivDialog = ({ empresaCodigo }: ImportNivDialogProps) => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formData.append("empresaCodigo", empresaCodigo);
      ...
      const response = await supabase.functions.invoke("import-niv", {
        body: formData,
      ...
      // Atualizar os dados
      queryClient.invalidateQueries({ queryKey: ["condicoes", empresaCodigo] });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Importar Condições Comerciais NIV</DialogTitle>
          <DialogDescription>
            Envie seu arquivo Excel (.xlsx ou .xls) com as condições comerciais da Empresa {empresaCodigo}
          </DialogDescription>
        </DialogHeader>
...
export default ImportNivDialog;
